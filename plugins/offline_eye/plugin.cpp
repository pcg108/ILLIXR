...
public:
    offline_eye(const std::string& name_, phonebook* pb_)
        ...
        , _m_eye_publisher_true{sb->get_writer<eye_type>("eye_raw")}
        , _m_eye_data{load_data()}
        , dataset_first_time{_m_eye_data.cbegin()->first}
        , last_ts{0}
        , _m_rtc{pb->lookup_impl<RelativeClock>()}
        , next_row{_m_eye_data.cbegin()} {
    ...
    void _p_one_iteration() override {
        ...
        auto after_nearest_row = _m_eye_data.upper_bound(lookup_time);

        if (after_nearest_row == _m_eye_data.cend()) {
#ifndef NDEBUG
            ...
            internal_stop();
        } else if (after_nearest_row == _m_eye_data.cbegin()) {
        ...
        if (last_ts != nearest_row->first) {
            ...
            auto eye_img = nearest_row->second.eye_img.load();
            auto true_eye_pos = nearest_row->second.eye_pos;

            time_point expected_real_time_given_dataset_time(
                std::chrono::duration<long, std::nano>{nearest_row->first - dataset_first_time});

            _m_eye_publisher_true.put(_m_eye_publisher_true.allocate<eye_type>(eye_type{
...
private:
    ...
    switchboard::writer<eye_type>                  _m_eye_publisher_true;
    const std::map<ullong, sensor_types>           _m_eye_data;
